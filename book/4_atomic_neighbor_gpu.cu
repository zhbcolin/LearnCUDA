...
void read_xy(std::vector<real>& v_x, std::vector<real>& v_y) {
    std::ifstream infile("../book/xy.txt");
...
__global__ void find_neighbor_atomic(int *d_NN, int *d_NL, const real *d_x, const real *d_y,
...
__global__ void find_neighbor_no_atomic(int *d_NN, int *d_NL, const real *d_x, const real *d_y,
                                        const int N, const real cutoff_square) {
    const int n1 = blockIdx.x * blockDim.x + threadIdx.x;
    if(n1 < N) {
        int count = 0;
...
void timing(int *d_NN, int *d_NL, const real *d_x, const real *d_y, const bool atomic) {
    for(int repeat = 0; repeat < NUM_REPEATS; ++repeat) {
        ...
        if(atomic) {
            find_neighbor_atomic<<<grid_size, block_size>>>(d_NN, d_NL, d_x, d_y, N, cutoff_square);
        } else {
            find_neighbor_no_atomic<<<grid_size, block_size>>>(d_NN, d_NL, d_x, d_y, N, cutoff_square);
        }
...
void print_neighbor(const int *NN, const int *NL, const bool atomic) {
    std::ofstream outfile("../book/neighbor.txt");
    if(!outfile) {
